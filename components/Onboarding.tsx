  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-4 animate-fade-in relative overflow-hidden pt-12 pb-12">
      ...
      <div className="text-center mb-10 max-w-md relative">
        ...
        <p className="text-gray-400 font-mono text-xs tracking-widest">SELECIONE O PROTOCOLO DE ANÁLISE</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 w-full max-w-md">
        {goals.map((goal) => (
          <button
            ...
            className="flex flex-col items-start p-4 bg-gray-900/50 backdrop-blur-md border border-gray-800 rounded-lg hover:border-cyan-500 hover:bg-gray-800 hover:shadow-[0_0_15px_rgba(6,182,212,0.2)] transition-all duration-300 group text-left relative overflow-hidden"
          >
            <div className="absolute top-0 right-0 w-16 h-16 bg-gradient-to-bl from-white/5 to-transparent -mr-8 -mt-8 rounded-full"></div>
            
            <div className="mb-3 p-2 bg-black border border-gray-800 rounded text-gray-400 group-hover:text-cyan-400 group-hover:border-cyan-500/50 transition-colors">
              {goal.icon}
            </div>
            <h3 className="font-bold text-gray-200 text-sm mb-1 font-mono tracking-tight">{goal.label}</h3>
            <p className="text-[10px] text-gray-500 leading-tight group-hover:text-gray-400">{goal.desc}</p>
          </button>
        ))}
      </div>
    </div>
